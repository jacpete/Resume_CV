...
position_data <- read_csv('positions.csv') %>% 
  ...
  mutate(start = parse_date_time(start, c("ymd", "y")), 
         end = parse_date_time(end, c("ymd", "y"))) %>% 
  filter(in_cv)
```
...
# When in export mode the little dots are unaligned, so fix that. 
if(PDF_EXPORT){
  cat("View this CV online with links at _https://github.com/jacpete/Resume_CV_")
} else {
  cat("[<i class='fas fa-download'></i> Download a PDF of this CV](https://github.com/jacpete/Resume_CV/blob/master/cv_Peterson.pdf)")
}
```
